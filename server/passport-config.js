...
if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_ID !== 'your_google_client_id') {
  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      let user = await db.prepare('SELECT * FROM patients WHERE google_id = ?').get(profile.id);

      if (!user) {
        // Vérifier si l'email existe déjà
        user = await db.prepare('SELECT * FROM patients WHERE email = ?').get(profile.emails[0].value);

        if (user) {
          // Mettre à jour avec le google_id
          await db.prepare('UPDATE patients SET google_id = ?, is_verified = 1 WHERE id = ?')
          ...
          user = await db.prepare('SELECT * FROM patients WHERE id = ?').get(result.lastInsertRowid);
        }
      }
...
if (process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_ID !== 'your_facebook_app_id') {
  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      // Chercher si l'utilisateur existe déjà
      let user = await db.prepare('SELECT * FROM patients WHERE facebook_id = ?').get(profile.id);

      if (!user) {
        // Vérifier si l'email existe déjà
        const email = profile.emails && profile.emails[0] ? profile.emails[0].value : `${profile.id}@facebook.com`;
        user = await db.prepare('SELECT * FROM patients WHERE email = ?').get(email);

        if (user) {
          // Mettre à jour avec le facebook_id
          await db.prepare('UPDATE patients SET facebook_id = ?, is_verified = 1 WHERE id = ?')
            .run(profile.id, user.id);
